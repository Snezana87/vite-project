...
async function extractStoriesJson(outputFile, storiesGlobs, configDir) {
  if (!storiesGlobs) {
  ...
  await Promise.all(storyFiles.map(async function (absolutePath) {
    ...
    try {
      var csf = (await (0, _csfTools.readCsfOrMdx)(absolutePath)).parse();
  ...
  await _fsExtra.default.writeJson(outputFile, {
    v: 3,
    stories: stories
  });
}
